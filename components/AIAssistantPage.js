                  ...
                  <div className="mt-6 group cursor-pointer">
                    <div className="relative aspect-[4/5] md:aspect-[16/10] rounded-[3rem] overflow-hidden mb-4 shadow-2xl isolation-auto">
                      ...
                      <div className="absolute bottom-8 left-8 right-8 text-left">
                        ...
                        <div className="bg-brand-amber text-white px-6 py-2.5 rounded-full text-sm font-black uppercase tracking-widest inline-block shadow-xl">{msg.tour.total}</div>
                      </div>
                    </div>
                    <p className="text-slate-500 text-base md:text-lg font-medium leading-relaxed px-4 text-left">{msg.tour.desc}</p>
                  </div>
                )}
              </div>
              {msg.suggestions && msg.suggestions.length > 0 && (
                <div className="flex flex-wrap gap-2 mt-3 mb-2 animate-in slide-in-from-left-4 duration-500">
                  {msg.suggestions.map((s, idx) => (
                    <button key={idx} onClick={() => handleSend(s.query)} className="px-4 py-2 bg-white border border-slate-100 rounded-xl text-xs md:text-sm font-bold text-slate-500 hover:border-brand-sky/30 hover:text-brand-indigo hover:bg-slate-50 transition-all active:scale-95 shadow-sm">
                      {s.label}
                    </button>
                  ))}
                </div>
        ...
        <div className="relative">
          <div className="flex flex-nowrap md:flex-wrap gap-2 overflow-x-auto no-scrollbar px-4 py-4 md:px-8 bg-white w-full border-b border-slate-50">
            {QUICK_ACTIONS.map((action, idx) => (
              <button key={idx} onClick={() => handleSend(action.query)} className="flex items-center gap-2 px-5 py-2.5 bg-white border border-slate-100 hover:border-brand-sky/30 hover:bg-slate-50 rounded-xl whitespace-nowrap transition-all active:scale-95 group shadow-sm shrink-0">
                <span className="text-base">{action.icon}</span>
                <span className="text-xs font-black text-slate-500 group-hover:text-brand-indigo transition-colors uppercase tracking-wider">{action.label}</span>
              </button>
            ))}
