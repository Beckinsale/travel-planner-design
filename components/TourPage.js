function TourPage({ tour, onBack, onStartChat }) {
  ...
  const tourDetails = {
    ...
    2: { // Алтай
      ...
      fullDesc: 'Алтай — это место силы, где природа сохранилась в своем первозданном виде. Вас ждут бирюзовые воды реки Катунь, величественные горы и перевалы Чуйского тракта. Мы пройдем по самым красивым точкам Горного Алтая, увидим Гейзерное озеро и посетим долину Чулышман. Это путешествие для тех, кто ищет уединения с природой и хочет почувствовать настоящую свободу вдали от городской суеты.',
      highlights: ['Чуйский тракт', 'Сплав по Катуни', 'Гейзерное озеро', 'Перевал Кату-Ярык'],
    },
    3: { // Байкал
      gallery: [
      ...
      fullDesc: 'Зимний Байкал — это бесконечная ледяная пустыня с самыми невероятными узорами, которые только может создать природа. Вы увидите знаменитые байкальские пузырьки, огромные торосы и кристально чистые ледяные пещеры. В программе — катание на коньках по самому большому катку мира, поездка на хивусе (катере на воздушной подушке) и знакомство с традициями шаманизма на острове Ольхон.',
      highlights: ['Лед острова Ольхон', 'Пузырьки в метане', 'Поездка на хивусе', 'Бурятская кухня'],
    },
    4: { // Камчатка
      gallery: [
  ...
  return (
    <div className="flex flex-col bg-white min-h-screen">
      {/* Navbar Overlay */}
      <div className="fixed top-0 left-0 right-0 z-[110] px-4 md:px-8 py-4 flex justify-between items-center bg-white/10 backdrop-blur-md border-b border-white/20">
        <button
          onClick={onBack}
                      ...
                      <div className="max-w-5xl mx-auto">
                        <div className="flex flex-wrap gap-2 mb-4">
                          {tour.tags.map((tag, idx) => (
                            <span key={idx} className="px-4 py-1.5 bg-brand-amber text-white text-[10px] font-black uppercase tracking-widest rounded-full shadow-lg">
                              {tag}
                            </span>
                          ))}
                        </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-16">
                      ...
                      <div className="lg:col-span-2">
                        ...
                        <div className="grid grid-cols-2 gap-4 mb-12">
                          {details.gallery.map((img, idx) => (
                            <div key={idx} className="aspect-square rounded-3xl overflow-hidden shadow-lg group">
                              <img
                                src={img}
                      ...
                      <div className="lg:col-span-1">
                        <div className="bg-slate-50 rounded-[2.5rem] p-8 md:p-10 sticky top-32">
                          ...
                          <div className="space-y-6 mb-10">
                            ...
                            <ul className="space-y-4">
                              {details.highlights.map((item, idx) => (
                                <li key={idx} className="flex items-start gap-3 text-slate-600 font-bold">
                          ...
                          <button
                            onClick={() => onStartChat(`Собери маршрут как в туре "${tour.title}"`)}
                          ...
                          <p className="text-center text-[10px] text-slate-400 font-bold mt-4 uppercase tracking-tighter">
                            Есть вопросы? <span className="text-brand-sky cursor-pointer" onClick={() => onStartChat(`Расскажи подробнее про тур "${tour.title}"`)}>Спроси AI</span>
                          </p>
                        </div>
                      </div>
                    </div>
